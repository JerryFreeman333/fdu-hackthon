...
因此理解层的肯否仲裁改为**LLM 优先、规则兜底**的双层结构（`engine/llmUnderstanding.ts`）：

```text
老人原话 → 分句
  ↓
① 规则层：结构化否定检测（否定语素 + 窗口 + 正面习语白名单）
   + 标签/数值抽取 + 人物归属 + 隐私边界（全部本地，永远执行）
  ↓
② LLM 仲裁（配置后启用）：对每个子句判断
   occurred / negated / hypothetical / uncertain / near_miss
  ↓
合并策略：肯否轴双向采纳 LLM 结论；
...
- 隐私意图为 `private` / `no_record` 的输入**不发送给外部模型**，肯否判断交给本地规则；
- LLM 只仲裁肯否这一个轴，不碰标签、数值、人物归属、分级与回复。
...
- **理解层 LLM 仲裁**（`llm-understanding.test.ts`）：mock fetch 验证肯否双向合并、强词汇信号保留、超时/HTTP 错误/坏 JSON/网络异常全部回落规则、隐私门控、超长输入跳过；
...
不配置即纯规则模式。配置后老人说"今天不太喘了"这类话由真实语言模型判定为"症状消失"，不再被记成正在发生；LLM 不可用时自动回落规则结果。
