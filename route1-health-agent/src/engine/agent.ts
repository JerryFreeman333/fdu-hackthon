...
export async function generateAgentReply(
  ...
  const safetyFinding = context?.priorityFindings.find((finding) => finding.severity === 'urgent' || finding.severity === 'alert');
  const safetyGuard = safetyFinding ? `当前最高风险等级为 ${safetyFinding.severity}，不要自行提高或降低等级。` : '当前没有更高等级安全信号。';
