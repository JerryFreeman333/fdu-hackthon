...
const INTENT_RULES: IntentRule[] = [
  ...
  {
    tag: 'dizziness',
    patterns: [/头晕/, /头.{0,2}晕/, /头昏/, /站不稳/, /走路不稳/, /走不稳/, /眼前发黑/, /天旋地转/],
...
export const ruleBasedAdapter: LlmAdapter = {
  async complete(_systemPrompt, userText, context) {
    ...
    return {
      text: buildRuleBasedReply(parsed.tags, [], parsed.tags.includes('fall'), context),
      tags: parsed.tags,
    };
  },
};
...
function sanitizeExternalContext(context: AgentContext): ExternalAgentContext {
  ...
  const safetyRank: Record<Finding['severity'], number> = {
    urgent: 0,
    alert: 1,
    watch: 2,
    info: 3,
...
const SYMPTOM_TAG_SET = new Set<SymptomTag>([
  'fatigue',
  'dyspnea',
  'poorSleep',
  'edema',
...
export function parseExternalLlmPayload(payload: unknown): { text: string; tags: SymptomTag[] } | null {
  ...
  return {
    ...
    tags: Array.isArray(candidate.tags) ? [...new Set(candidate.tags as SymptomTag[])] : [],
  };
}
