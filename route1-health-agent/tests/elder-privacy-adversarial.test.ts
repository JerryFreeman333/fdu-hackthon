...
runCase('conflicting share and refusal fails closed', () => {
  ...
  assert(
    ...
    'mixed share and refusal must fail closed to private',
  );
  assert(
    !canShareWithFamily('granted', parsePrivacyIntent(text)),
    'conflicting privacy command must not share',
  );
});
...
runCase('no-record remains stronger than conflicting sharing language', () => {
  ...
  assert(
    parsePrivacyIntent(text) === 'no_record',
    'no-record must remain the strongest safety boundary',
  );
  assert(
    !canShareWithFamily('granted', parsePrivacyIntent(text)),
    'no-record must block sharing',
  );
});
...
runCase('a clean one-time request remains shareable', () => {
  ...
  assert(
    intent === 'share_family',
    'clean one-time share request should remain shareable',
  );
  assert(
    canShareWithFamily('denied', intent),
    'explicit one-time share may override persistent denial',
  );
});

runCase('mixed privacy claims produce no persistence candidates', () => {
  const input = understandElderInput(
    '告诉女儿我的头晕，但是爸爸的事情不要告诉她',
    TODAY,
  );
  assert(
    input.claims.length === 0,
    'mixed privacy input must expose no claims to persistence',
  );
  assert(
    Boolean(input.clarificationQuestion),
    'mixed privacy input must ask for clarification',
  );
  assert(
...
runCase('revocation removes persistent family visibility', () => {
  ...
  assert(
    visibleFamilyEvents([event], 'granted').length === 1,
    'granted state should expose persistent event',
  );
  assert(
