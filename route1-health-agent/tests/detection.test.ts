...
import type { DayRecord, Observation } from '../src/types';
...
import { generateAgentReply } from '../src/engine/agent';
...
function runCase(name: string, fn: () => void): void { fn(); console.log(`PASS: ${name}`); }

runCase('demo event stream produces multisignal alert', () => {
...
runCase('identical event stream produces stable findings', () => {
...
runCase('sparse recent data does not create a trend', () => {
...
runCase('single weak metric stays at watch', () => {
...
runCase('severe blood pressure without red flags stays alert', () => {
...
runCase('severe blood pressure plus chest pain upgrades to urgent', () => {
...
runCase('fall directly routes to urgent', () => {
...
runCase('private observation prevents family escalation without hiding it from the elder', () => {
...
runCase('private red-flag symptom remains urgent but is not copied to family', () => {
...
runCase('privacy intent is explicit and conservative', () => {
...
runCase('justified follow-up question is driven by context', () => {
  ...
  const reply = generateAgentReply('最近腿有点没劲', ['fatigue'], findings, false, context);
  assert(reply.includes(questions[0].question), 'Agent reply should actually use the justified question policy');
});

runCase('care tasks are contextual rather than daily noise', () => {
  ...
  const findings = runDetection(recordsToEvents(makeRecords(
...
runCase('all three health input types materialize from one stream', () => {
...
runCase('agent context is bounded and carries Person Twin evidence', () => {
