# 路线二 —— 一键全流程：数据准备 → COLMAP → 视觉语义/3D 定位 → 候选拓扑 → 语义可通行候选 → 个体化通行 → Person × Home 风险 → 3DGS → 导出 Web
# 用法1(视频): powershell -ExecutionPolicy Bypass -File scripts\run_all.ps1 -Video "C:\path\home.mp4" -Scene home -PersonProfile "C:\path\person.json" -ScaleFile "C:\path\scale.json" -MobilityProfile "C:\path\mobility.json"
...
if ($MobilityProfile) { $surfaceArgs += @('-ClearanceProfile', $MobilityProfile) }
